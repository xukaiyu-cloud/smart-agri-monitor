...
// 确定性伪随机：同一(hour, pt)始终返回同一偏移量
static double dnoise(double hour, int pt, double scale) {
    double seed = sin(hour * 47.123 + pt * 13.579) * 10000;
    return (seed - floor(seed)) * scale * 2.0 - scale;
}
...
double sim_hour_now() {
    ...
    time_t sim_now = g_start + (time(nullptr) - g_start) * 300;
    time_t ts = (time_t)sim_now;
...
static SensorData make_sensor(int pt, double hour) {
    ...
    double temp = b[0] + day*7 + dnoise(hour, pt, 0.3);
    double hum  = 65 - (temp-b[0])*3 + dnoise(hour, pt+5, 1.2);
    int light;
    if(hour>5.5 && hour<19) {
        double sun = sin((hour-5.5)/13.5*3.14159);
        light = (int)(sun*b[2] + dnoise(hour, pt+10, b[2]*0.08));
        if(uni(rng)<0.15) light = (int)(light*(0.65+uni(rng)*0.25));
    } else light = (int)(uni(rng)*8);
    double ph = b[3] + dnoise(hour, pt+15, 0.04);
...
SensorData sim_sensor_at_hour(int pt, double hour) {
    double h = fmod(hour, 24.0);
    if (h < 0) h += 24.0;
    return make_sensor(pt, h);
}